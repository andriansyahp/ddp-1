...
class BST:
    ...
    def find_node(self, current_node, value):
        if (current_node is None):
            ...
            self.trace.append(str(current_node.value))
            return True
        elif (current_node.value < value):
            self.trace.append(str(current_node.value))
        ...
        # implementasikan code ini menggunakan recursion
        ...
        # kemudian menambahkan setiap node yang terlewati ke dalam list trace
...
def main():
    ...
    # Meminta input dari user, angka yang yang akan dicek apakah ada dalam tree
        ...
        if(bst.find(x)):
            ...
            print("Node yang terlewati : {}".format(' '.join(bst.trace)))   # mencetak trace
            bst.trace.clear()                                               # menghapus trace sebelumnya
        else:
